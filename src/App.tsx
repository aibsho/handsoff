...
function PostForm({ onPost, onClose }: { onPost: () => void; onClose: () => void }) {
  const [form, setForm] = useState<FormData>({ role: "", sector: "Healthcare", location: "", leaving_in: "", reason: "", notes: "", email: "" });
  ...
  async function handleSubmit() {
    ...
    const { error: err } = await supabase.from("posts").insert([{
      role: form.role, sector: form.sector, location: form.location,
      leaving_in: form.leaving_in, reason: form.reason, notes: form.notes,
      initials, saves: 0,
    }]);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 20 }}>
      <div style={{ background: "#141822", border: "1.5px solid #252b3b", borderRadius: 20, padding: "36px 32px", width: "100%", maxWidth: 520, maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ fontSize: 13, color: "#555e7a", marginBottom: 28 }}>Let the right person know before it's even advertised.</div>
        ...
        <div style={{ display: "flex", flexDirection: "column" as const, gap: 18 }}>
          ...
          <div>
            ...
            <textarea style={{ ...inputStyle, resize: "vertical", minHeight: 90 }} placeholder="What should the next person know?" value={form.notes} onChange={e => update("notes", e.target.value)} />
          </div>
          <div>
      ...
      <div style={{ background: "#141822", border: "1.5px solid #252b3b", borderRadius: 20, padding: "36px 32px", width: "100%", maxWidth: 420 }}>
        {done ? (
          <div style={{ textAlign: "center" as const }}>
            ...
            <div style={{ fontSize: 13, color: "#555e7a", marginBottom: 24 }}>We'll email you when a {sector} role is posted.</div>
            <button onClick={onClose} style={{ background: "#e05c2a", border: "none", borderRadius: 10, color: "#fff", fontFamily: "'DM Mono', monospace", fontSize: 14, fontWeight: 700, padding: "12px 32px", cursor: "pointer" }}>Done</button>
          </div>
        ) : (
          <>
            <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, color: "#f0ece0", marginBottom: 6 }}>Get alerts</div>
            <div style={{ fontSize: 13, color: "#555e7a", marginBottom: 24 }}>We'll notify you the moment someone posts a handoff in your sector.</div>
            <div style={{ display: "flex", flexDirection: "column" as const, gap: 14 }}>
              <div>
                <label style={{ fontSize: 11, color: "#555e7a", fontFamily: "'DM Mono', monospace", marginBottom: 6, display: "block" as const }}>YOUR EMAIL</label>
                <input style={inputStyle} placeholder="you@email.com" type="email" value={email} onChange={e => setEmail(e.target.value)} />
              </div>
              <div>
                <label style={{ fontSize: 11, color: "#555e7a", fontFamily: "'DM Mono', monospace", marginBottom: 6, display: "block" as const }}>SECTOR</label>
    ...
    <div style={{ minHeight: "100vh", background: "#0d1018" }}>
      ...
      <div style={{ borderBottom: "1.5px solid #1a1f2e", padding: "0 24px" }}>
        <div style={{ maxWidth: 760, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => setShowAlert(true)} style={{ background: "transparent", border: "1.5px solid #252b3b", borderRadius: 10, color: "#8a93b0", fontFamily: "'DM Mono', monospace", fontSize: 13, fontWeight: 700, padding: "9px 16px", cursor: "pointer" }}>🔔 Alerts</button>
            <button onClick={() => setShowForm(true)} style={{ background: "#e05c2a", border: "none", borderRadius: 10, color: "#fff", fontFamily: "'DM Mono', monospace", fontSize: 13, fontWeight: 700, padding: "9px 20px", cursor: "pointer" }}>+ I'm leaving</button>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "40px 24px" }}>
        <div style={{ marginBottom: 40 }}>
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(28px, 5vw, 42px)", fontWeight: 900, color: "#f0ece0", lineHeight: 1.15, margin: "0 0 12px" }}>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" as const }}>
          <input
            style={{ flex: 1, minWidth: 180, background: "#141822", border: "1.5px solid #252b3b", borderRadius: 10, padding: "10px 14px", color: "#f0ece0", fontFamily: "'DM Sans', sans-serif", fontSize: 14, outline: "none" }}
        ...
        <div style={{ display: "flex", gap: 24, marginBottom: 28, padding: "14px 20px", background: "#141822", borderRadius: 12, border: "1.5px solid #1a1f2e" }}>
          {[
            ...
            { label: "Healthcare", value: posts.filter(p => p.sector === "Healthcare").length },
        ...
        <div style={{ display: "flex", flexDirection: "column" as const, gap: 16 }}>
          {loading ? (
            ...
            <div style={{ textAlign: "center" as const, color: "#555e7a", padding: "60px 0", fontFamily: "'DM Mono', monospace", fontSize: 13 }}>No posts yet — be the first to post a handoff. 👆</div>
          ) : filtered.map(post => (
            <Card key={post.id} post={post} onSave={handleSave} savedIds={savedIds} />
          ))}
        </div>

        <div style={{ marginTop: 48, padding: "32px", background: "#141822", border: "1.5px solid #252b3b", borderRadius: 16, textAlign: "center" as const }}>
          <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, color: "#f0ece0", marginBottom: 8 }}>Leaving a role soon?</div>
          <div style={{ fontSize: 13, color: "#555e7a", marginBottom: 20 }}>Give someone else the jump. Post anonymously in 60 seconds.</div>
          <button onClick={() => setShowForm(true)} style={{ background: "#e05c2a", border: "none", borderRadius: 10, color: "#fff", fontFamily: "'DM Mono', monospace", fontSize: 14, fontWeight: 700, padding: "12px 32px", cursor: "pointer" }}>Post your handoff →</button>
        </div>
      </div>
